...
#define EFFECT_TIME_ANIMATION			(10)	// アニメーションの切り替わるカウント
		...
		{									// Yes
			...
			for (int n = 0; n < effectWk[i].effectCount; n++)
			{
				if (effectWk[i].pParticle[n].isFinish == 0) {
					//バレットの位置やテクスチャー座標を反映
					float px = effectWk[i].pParticle[n].pos.x;	// エフェクトの表示位置X
					float py = effectWk[i].pParticle[n].pos.y;	// エフェクトの表示位置Y
					float pw = EFFECT_TEXTURE_SIZE_X;			// エフェクトの表示幅
					float ph = EFFECT_TEXTURE_SIZE_Y;			// エフェクトの表示高さ

					px -= EFFECT_TEXTURE_SIZE_X / 3;
					py -= EFFECT_TEXTURE_SIZE_Y / 2;
